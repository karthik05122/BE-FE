  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchExecutiveOrders.fulfilled, (state, action) => {
        ...
        state.executiveOrders = Array.isArray(action.payload) ? action.payload : [];
        state.error = null;
      })
      ...
      .addCase(fetchExecutiveOrderDetails.fulfilled, (state, action) => {
        state.selectedEO = action.payload;
        state.error = null;
      })
      ...
      .addCase(fetchEmailLogs.fulfilled, (state, action) => {
        // Ensure we always have an array, even if the API returns something unexpected
        state.emailLogs = Array.isArray(action.payload) ? action.payload : [];
        state.error = null;
      });
  },
});
